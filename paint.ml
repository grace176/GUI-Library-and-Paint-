...
type state = {
  ...
  (** The preview of the drawn shape *)
...
let repaint (g: gctx) : unit =
  let draw_shape (s: shape) : unit =
    begin match s with
      ...
      | Ellipse e -> 
        let (x, y) = (fst (e.p1) + fst (e.p2))/2, (snd (e.p1) + snd (e.p2))/2 in
        let rx, ry = (abs(fst e.p1 - fst e.p2))/2, (abs(snd e.p1 - snd e.p2))/2
        in draw_ellipse (with_params g e.color false) (x, y) rx ry
    end in
  ...
  begin match (event_type event) with
    | MouseDown ->
      ...
      (begin match paint.mode with 
         ...
         | PointMode -> 
           let new_points = Points {color = paint.color; points = [p]} in
           paint.preview <- Some new_points
         | EllipseStartMode ->
    ...
    | MouseUp ->
      ...
      | LineEndMode p' -> 
        Deque.insert_tail (Line 
          {color=paint.color; p1=p'; p2=p; thickness = paint.thick}) 
          paint.shapes;
      ...
      | EllipseEndMode p' ->
        Deque.insert_tail (Ellipse {color=paint.color; p1=p; p2=p'}) 
          paint.shapes;
...
let undo () : unit =
  if Deque.is_empty paint.shapes then () else
    ...
    | PointMode -> paint.mode <- PointMode
    | EllipseStartMode -> ()
...
let thick_box, thick_vc = checkbox false "THICK" 
;;thick_vc.add_change_listener (fun b -> paint.thick <- b)
...
let mode_toolbar : widget =
  let mode_list = [w_undo; spacer; w_line; spacer; w_point; 
                   spacer; w_ellipse; spacer; thick_box] 
  in
  hlist mode_list

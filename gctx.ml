...
(** Produce a new Gctx.t with a different thickness *)
let with_thickness (g: gctx) (t: bool) : gctx = 
  { g with thick = t }
...
let set_graphics_state (gc: gctx) : unit =
  ...
  Graphics.set_line_width (if gc.thick then 4 else 1)
...
let draw_point (g: gctx) (coor: position) : unit = 
  set_graphics_state g;
  let (x1, y1) = ocaml_coords g coor in
  Graphics.plot x1 y1 
 
let draw_points (g: gctx) (p_list: position list) : unit = 
  set_graphics_state g;
  List.iter (fun p -> draw_point g p) p_list
